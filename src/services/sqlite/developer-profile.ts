...
export function getProfileByProject(
  db: Database,
  project: string,
...
export function updateProfileConfidence(
  db: Database,
  id: number,
  confidence: number,
  evidenceCount: number,
  ...
  if (sourceReflectionIds) {
    db.query(
      'UPDATE developer_profile SET confidence = ?, evidence_count = ?, source_reflection_ids = ?, updated_at_epoch = ? WHERE id = ?',
    ).run(confidence, evidenceCount, sourceReflectionIds, now, id);
  } else {
    db.query('UPDATE developer_profile SET confidence = ?, evidence_count = ?, updated_at_epoch = ? WHERE id = ?').run(
      confidence,
      evidenceCount,
      now,
      id,
...
export function findSimilarProfileEntry(
  db: Database,
  project: string | null,
  category: string,
  descriptionFragment: string,
): ProfileEntryRow | null {
  ...
  return db
    .query(`SELECT * FROM developer_profile WHERE ${projClause} AND category = ? AND description LIKE ? LIMIT 1`)
...
export function countProfileEntries(db: Database, project?: string): number {
  if (project) {
    return (
      db
        .query('SELECT COUNT(*) as count FROM developer_profile WHERE project = ? OR project IS NULL')
        .get(project) as any
    ).count;
  }
