...
export function upsertEntity(
  ...
  if (existing) {
    db.run(
      'UPDATE entities SET mention_count = mention_count + 1, last_seen_epoch = ? WHERE id = ?',
      [now, existing.id]
    );
    return existing.id;
  }
...
export function getEntitiesByProject(
  ...
  if (opts?.entityType) {
    return db.query(
      'SELECT * FROM entities WHERE project = ? AND entity_type = ? ORDER BY last_seen_epoch DESC LIMIT ?'
    ).all(project, opts.entityType, limit) as EntityRow[];
  }

  return db.query(
    'SELECT * FROM entities WHERE project = ? ORDER BY last_seen_epoch DESC LIMIT ?'
 ...
 */
export function getEntitiesByObservation(
  db: Database,
...
export function getHotspotEntities(
  ...
  if (opts?.entityType) {
    return db.query(
      'SELECT * FROM entities WHERE project = ? AND entity_type = ? ORDER BY mention_count DESC LIMIT ?'
    ).all(project, opts.entityType, limit) as EntityRow[];
  }

  return db.query(
    'SELECT * FROM entities WHERE project = ? ORDER BY mention_count DESC LIMIT ?'
  ).all(project, limit) as EntityRow[];
}
