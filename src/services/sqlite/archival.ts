...
export function archiveOldObservations(
  ...
  const result = db.transaction(() => {
    ...
    const insertResult = db.run(
      `INSERT INTO archived_observations
         (id, session_id, project, branch, source_ide, type, title, facts, concepts, files_affected, importance, prompt_number, created_at, created_at_epoch, archived_at_epoch)
       SELECT
         id, session_id, project, branch, source_ide, type, title, facts, concepts, files_affected, importance, prompt_number, created_at, created_at_epoch, ?
       FROM observations
    ...
    if (archivedCount > 0) {
      db.run(
        'DELETE FROM observations WHERE project = ? AND created_at_epoch < ?',
        [project, cutoffEpoch]
      );
    }
...
export function getArchivalStats(
  ...
  const archivedRow = db.query(
    'SELECT COUNT(*) as count FROM archived_observations WHERE project = ?'
  ).get(project) as { count: number };

  const oldestRow = db.query(
    'SELECT MIN(created_at_epoch) as oldest FROM observations WHERE project = ?'
