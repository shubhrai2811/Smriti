...
function createTestSession(db: any, project: string = '/tmp/test-project', status: string = 'completed', sourceIde: string = 'claude-code'): number {
  db.query(
  ...
  describe('Data API: Observations', () => {
    ...
    it('GET /data/observations returns observations', async () => {
      ...
      insertObservation(ctx.db, {
        sessionId: session, project: '/tmp/proj', type: 'discovery',
        title: 'Found the bug', importance: 7,
      });
      insertObservation(ctx.db, {
        sessionId: session, project: '/tmp/proj', type: 'bugfix',
        title: 'Fixed the bug', importance: 8,
      });
    ...
    it('GET /data/observations filters by project', async () => {
      ...
      insertObservation(ctx.db, {
        sessionId: s1, project: '/tmp/proj-a', type: 'discovery',
        title: 'Obs A', importance: 5,
      });
      insertObservation(ctx.db, {
        sessionId: s2, project: '/tmp/proj-b', type: 'discovery',
        title: 'Obs B', importance: 5,
      });
    ...
    it('GET /data/observations filters by branch', async () => {
      const session = createTestSession(ctx.db, '/tmp/proj');
      insertObservation(ctx.db, {
        sessionId: session, project: '/tmp/proj', branch: 'main',
        type: 'discovery', title: 'Main obs', importance: 5,
      });
      insertObservation(ctx.db, {
        sessionId: session, project: '/tmp/proj', branch: 'feature',
    ...
    it('observation response includes expected fields', async () => {
      ...
      insertObservation(ctx.db, {
        sessionId: session, project: '/tmp/proj', type: 'bugfix',
        title: 'Fix crash', facts: '["app crashes on start"]',
        concepts: '["error handling"]', filesAffected: '["main.ts"]',
        importance: 9,
      });
  ...
  describe('Data API: Reflections', () => {
    ...
    it('GET /data/reflections returns reflections', async () => {
      ...
      insertReflection(ctx.db, {
        sessionId: session, project: '/tmp/proj', type: 'quick',
        insight: 'Tests should run fast', category: 'pattern',
      });
      insertReflection(ctx.db, {
        sessionId: session, project: '/tmp/proj', type: 'deep',
        insight: 'Architecture needs refactoring', category: 'improvement',
      });
    ...
    it('GET /data/reflections filters by project', async () => {
      ...
      const s2 = createTestSession(ctx.db, '/tmp/proj-b');
      insertReflection(ctx.db, {
        sessionId: s1, project: '/tmp/proj-a', type: 'quick',
        insight: 'Insight A',
      });
      insertReflection(ctx.db, {
        sessionId: s2, project: '/tmp/proj-b', type: 'quick',
        insight: 'Insight B',
      });
  ...
  describe('Data API: Profile', () => {
    ...
    it('GET /data/profile returns profile entries', async () => {
      insertProfileEntry(ctx.db, {
        project: '/tmp/proj', category: 'preference',
        description: 'Prefers functional style',
      });
      insertProfileEntry(ctx.db, {
        project: '/tmp/proj', category: 'pattern',
    ...
    it('profile entry includes expected fields', async () => {
      insertProfileEntry(ctx.db, {
        project: '/tmp/proj', category: 'style',
        description: 'Minimal comments', confidence: 0.8,
        evidenceCount: 5, sourceReflectionIds: '[1, 2]',
      });
  ...
  describe('Data API: Links', () => {
    ...
    it('GET /data/links returns empty for observation with no links', async () => {
      ...
      const obsId = insertObservation(ctx.db, {
        sessionId: session, project: '/tmp/proj', type: 'discovery',
        title: 'Lonely observation', importance: 5,
      });
    ...
    it('GET /data/links returns linked observations', async () => {
      ...
      const obs1 = insertObservation(ctx.db, {
        sessionId: session, project: '/tmp/proj', type: 'discovery',
        title: 'Found issue', importance: 7,
      });
      const obs2 = insertObservation(ctx.db, {
        sessionId: session, project: '/tmp/proj', type: 'bugfix',
        title: 'Fixed issue', importance: 8,
      });

      insertLink(ctx.db, {
        sourceId: obs1, targetId: obs2, linkType: 'fixed_by', confidence: 0.9,
      });
